...
conjNotInjOnto (Just fun) _ pr =
	...
	existsFun "F" fun $ \f -> 
		let subset = \x -> exist w (f w `eq` x) in
			-- we can always use the subset on which the function is surjective.
		case pr of
			Nothing ->  
			
				forEvery x (
					(nt (subset x)) \/ subset (f x) --p closed under f
				) /\
				...
				/\
				forEvery y ((nt (subset y)) \/ exist x ( subset x /\    --f surjective
				(f x `eq` y)))

			Just p' -> 
				existsPred "P" p' $ \q ->	
				let p = \x -> q x /\ subset x in			
					forEvery x (
