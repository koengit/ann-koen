...
equalify theory oblig = do
 ...
 let 
  ...
  wmap    = filter ((== "right_euclidean").show.snd) pmap
  tmap    = filter ((== Transitive).snd) pmap
 ...
 let 
  ...
  (method,rels) = (case ify of
    ...
    Flags.Ordify    -> (strictifyAll, transformRels ify noClash 0 (totalorders propmap))
  ...
  wonkrs  = wonkyrs \\ trs --(map fst wonkyrs) \\ (map fst trs)
...
--	 tffs = clauses2tff  (map toClause (filter isClause es))
	 ...
	 decls = if ify == Tordify then Just (map typeDecl (symbs++newsymbs)) else Nothing
    -- putStrLn $ show 
...
--     putStrLn $ tffs
  --   writeProblemToFile outfile decls (map Cl cs) Ordify ss  
     writeProblemToFile outfile decls es Equalify ss  
     if length wonkrs > 0  then appendFile "hej" (tf ++ "\n" ++ show wonkrs ++ "\n" ++ show trs ++ "\n") else return ()
...
replacePropertyOr r p c newc fun = case withProperty p c of
  Just r' -> if (rsymbol (relit r)) == rsymbol r' then Right [newc] --if (relit r) =*= r' then Right [newc]
	                           else Left $ fun r c
  Nothing -> Left $ fun r c
...
strictifyAll :: ClausesOrComments -> [RelationTransformation] -> ClausesOrComments
strictifyAll c rs = strictify c (head rs)

strictify :: ClausesOrComments -> RelationTransformation -> ClausesOrComments
strictify cs rt = onClauses (fun rt) cs
  where fun rt = \c -> case negateOrder rt c of
	            Left c'  -> Left c' 
		    Right c' -> Left c'


negateOrder :: RelationTransformation -> Clause -> Either [Clause] [Clause]
negateOrder r c = replacePropertyOr r Transitive c c (negateWithR) 


negateWithR :: RelationTransformation -> Clause -> [Clause]
negateWithR r c = [map (negateLiteralWithR (relit r)) c]	

negateLiteralWithR r l = case the l of
		((Fun f xs) :=: true) -> if f == (rsymbol r) then trace ("negating " ++ show l) $ negat l else l
		_ -> l
