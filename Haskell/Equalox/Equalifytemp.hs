...
clausesOrComments2format f ((Cl c):cs) n = 
   (if f == Ordify then clause2tff else clause2cnf)  c n : clausesOrComments2format f cs (n+1)
...
equalify theory oblig = do
 ...
 let 
  ...
  do 
  if rels /= [] then do
     let es = method (map Cl cs) (nubbed rels)
...
--     putStrLn $ tffs
  ...
  printPropMap propmap tf
  return $ NoAnswerClause NA
  
  

printPropMap :: PropertyMap -> FilePath -> IO ()
printPropMap pm tf = do
   --mPutStrLn ((tf) ++ "   " ++ show ( (length (eqrels pm)) `div` 2)) (eqrels pm)
