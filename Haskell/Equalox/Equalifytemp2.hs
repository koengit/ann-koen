...
--	 tffs = clauses2tff  (map toClause (filter isClause es))
	 ...
	 decls = if ify == Tordify || ify == Ordify then Just (map typeDecl (symbs++newsymbs)) else Nothing
...
transifyLiteral r k lit = 
 if not (isInteresting lit (rsymbol (relit r))) then [lit] else 
   case rsign (relit r) of
    ...
    Just b -> if sign lit /= b then  [Neg (the (changeSymbol lit (newsymbol r)))] 
	       else 
		  let varx = makeNewVar r k 
		     
		      newlits = [Neg (the (lit @@ varx)), Pos (the (lit @@@ varx))]
		  in
